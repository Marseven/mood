...
class TrackModel extends Model {
    ...
    public function findTrack($trackId, $approved = true) {
        ...
        if ($approved) {
            if (is_numeric($trackId)) {
                $query = $this->db->query("SELECT * from tracks WHERE  (id=?) AND status=? AND ((public != ? AND public!=?) OR userid=?) AND approved=?",$trackId, 1, 3,2,  $userid, 1);
            } else {
                $query = $this->db->query("SELECT * from tracks WHERE  (download_hash=?) AND status=? AND ((public != ? AND public!=?) OR userid=?) AND approved=?",$trackId, 1, 3,2,  $userid, 1);
            }
        } else {
            if (is_numeric($trackId)) {
                $query = $this->db->query("SELECT * from tracks WHERE  (id=?) AND status=? AND (public != ? OR userid=?) ",$trackId, 1, 3, $userid);
            } else {
                $query = $this->db->query("SELECT * from tracks WHERE  (download_hash=?) AND status=? AND (public != ? OR userid=?) ",$trackId, 1, 3, $userid);
            }
        }
    ...
    public function listPlaylists($type, $typeId, $limit = 20, $offset = 0) {
        ...
        if ($typeId == 'collection') {
            $sql = "SELECT * FROM playlist WHERE (userid=? OR id IN (SELECT typeid FROM likes WHERE type=? AND typeid=playlist.id AND userid=? )) AND playlist_type=?";
            $sql .= " ORDER BY id DESC LIMIT $limit OFFSET $offset";
            $playlistType = ($type == 'playlist') ? 1 : 0;
            $query = $this->db->query($sql, $userid, 'playlist', $userid, $playlistType);
            return $query->fetchAll(PDO::FETCH_ASSOC);
        } elseif ($typeId == 'discover') {
            $sql = "SELECT *,(SELECT count(typeid) as types FROM likes WHERE type=? AND typeid=playlist.id) as count FROM playlist WHERE  playlist_type=? AND public = ?";
            $sql .= " ORDER BY id DESC LIMIT $limit OFFSET $offset";
            $playlistType = ($type == 'playlist') ? 1 : 0;
            ...
            $sql = "SELECT * FROM playlist WHERE (userid=?) AND playlist_type=? AND (public != ? OR userid=?)  AND (public !=? OR userid=?)";
            ...
            $sql = "SELECT * FROM playlist WHERE (name LIKE ? OR description LIKE ? ) AND playlist_type=? AND public =? ";
            $sql .= " ORDER BY time DESC LIMIT $limit OFFSET $offset";
            $term = "%$term%";
            $playlistType = ($type == 'playlist') ? 1 : 0;
